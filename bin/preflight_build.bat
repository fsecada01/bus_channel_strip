...
echo.
echo Environment variables for bindgen:
  echo BINDGEN_EXTRA_CLANG_ARGS=%BINDGEN_EXTRA_CLANG_ARGS%
  echo CLANG_ARGS=%CLANG_ARGS%
  echo LIBCLANG_PATH=%LIBCLANG_PATH%
  echo CC=%CC%
  echo CXX=%CXX%
REM Set additional environment variables that skia-bindings might use
REM The CFLAGS/CXXFLAGS variables are parsed incorrectly by the 'cc' crate when they
REM contain quoted paths. The build scripts for this crate and for skia-bindings
REM now handle this manually or through their own detection logic.
REM set "CFLAGS=%BINDGEN_EXTRA_CLANG_ARGS%"
REM set "CXXFLAGS=%BINDGEN_EXTRA_CLANG_ARGS%"
REM set "CPPFLAGS=%BINDGEN_EXTRA_CLANG_ARGS%"
REM Try alternative environment variables for bindgen
set "RUST_BACKTRACE=1"
set "LIBCLANG_STATIC=1"
cargo xtask bundle bus_channel_strip --release --features api5500,buttercomp2,transformer,gui
if errorlevel 1 (
  echo.
