...
<span class="module-dyneq">**4-band frequency-dependent dynamics with spectral analyzer**</span>
...
Three modes give complete flexibility:
...
- **Gain reduction meters** — per-band GR in dB displayed at the top of the spectrum
...
| **Detector Freq** | 20 Hz – 20 kHz | Sidechain filter center. Envelope follower measures level at this frequency, enabling triggering independent of the EQ band's own center. |
...
| **Makeup Gain** | −12 – +12 dB | Static gain after dynamic processing. Compensate for GR or add band-specific level. |
| **Solo** | On/Off | Routes only this band through a bandpass filter for monitoring. Multiple bands can be soloed. |
...
- Sidechain detection uses a +6 dB peaking filter at the detector frequency — frequency-specific envelope following
- Gain hysteresis (0.05 dB) prevents redundant coefficient recomputation during stable envelope periods
- Solo mode updates envelope state for non-soloed bands so they respond instantly when solo is released
- Gain reduction is displayed via lock-free `AtomicU32` per band, updated from the audio thread with Relaxed ordering
