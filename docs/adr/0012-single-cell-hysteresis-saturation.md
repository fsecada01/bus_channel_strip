...
Integration points:
- **Transformer** (`TransformerStage::process_sample`): the play operator runs on each oversampled sample, between the drive stage and the model-specific saturation curve (`saturate_by_model`). A new `transformer_hysteresis_bypass` BoolParam (default `false` — hysteresis ON) skips the cell entirely when bypassed, making that path bit-identical to pre-#16 v1.0 output — the migration requirement from roadmap §6, since this is the bigger, default-on sound change.
- **Sheen WARMTH** (`SheenModule::process_warmth`): a new `sheen_warmth_tape_mode` BoolParam (default `false` — off) optionally routes the oversampled signal through the cell before the Inflator polynomial. Default off means WARMTH's factory sound is unchanged unless a user opts in — no migration note needed here, unlike Transformer.
