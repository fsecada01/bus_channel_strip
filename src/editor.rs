...
fn draw_dynamic_eq_module(ui: &mut Ui, params: &BusChannelStripParams) {
    ...
    ui.allocate_ui_at_rect(module_rect, |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                ui.add_space(8.0);
                draw_bypass_button(ui, &params.dyneq_bypass, DYNEQ_ACCENT);
            });
        });
        ...
        ui.horizontal(|ui| {
            ...
            // Band 1
            draw_dyneq_band(ui, params, 1, DYNEQ_ACCENT);
            draw_dyneq_band(ui, params, 2, DYNEQ_ACCENT);
            draw_dyneq_band(ui, params, 3, DYNEQ_ACCENT);
            draw_dyneq_band(ui, params, 4, DYNEQ_ACCENT);
        });
    });
}

/// Draw single Dynamic EQ band
fn draw_dyneq_band(ui: &mut Ui, params: &BusChannelStripParams, band: u8, accent_color: Color32) {
    ui.vertical(|ui| {
        ui.label(RichText::new(format!("B{}", band)).size(10.0).color(accent_color));
        
        // Get parameters for this band
        let (freq, thresh, ratio, attack, release, gain, q, enabled) = match band {
            1 => (&params.dyneq_band1_freq, &params.dyneq_band1_threshold, &params.dyneq_band1_ratio, 
                  &params.dyneq_band1_attack, &params.dyneq_band1_release, &params.dyneq_band1_gain, 
                  &params.dyneq_band1_q, &params.dyneq_band1_enabled),
            2 => (&params.dyneq_band2_freq, &params.dyneq_band2_threshold, &params.dyneq_band2_ratio, 
                  &params.dyneq_band2_attack, &params.dyneq_band2_release, &params.dyneq_band2_gain, 
                  &params.dyneq_band2_q, &params.dyneq_band2_enabled),
            3 => (&params.dyneq_band3_freq, &params.dyneq_band3_threshold, &params.dyneq_band3_ratio, 
                  &params.dyneq_band3_attack, &params.dyneq_band3_release, &params.dyneq_band3_gain, 
                  &params.dyneq_band3_q, &params.dyneq_band3_enabled),
            4 => (&params.dyneq_band4_freq, &params.dyneq_band4_threshold, &params.dyneq_band4_ratio, 
                  &params.dyneq_band4_attack, &params.dyneq_band4_release, &params.dyneq_band4_gain, 
                  &params.dyneq_band4_q, &params.dyneq_band4_enabled),
            _ => return,
        };
        
        // Compact controls
        draw_mini_knob(ui, freq, "F", accent_color);
        draw_mini_knob(ui, thresh, "T", accent_color);
        draw_mini_knob(ui, ratio, "R", accent_color);
        draw_mini_knob(ui, gain, "G", accent_color);
        
        ui.add_space(5.0);
        draw_mini_button(ui, enabled, "ON", accent_color);
    });
...
fn draw_knob(ui: &mut Ui, param: &impl Param, label: &str, color: Color32, size: f32) {
    ui.vertical_centered(|ui| {
        ...
        let response = ui.add_sized([size, size], 
            egui::Slider::new(&mut param.normalized_value(), 0.0..=1.0)
                .show_value(false)
        );
        ...
        // Knob indicator
        let angle = (param.normalized_value() - 0.5) * 2.0 * std::f32::consts::PI * 0.75;
