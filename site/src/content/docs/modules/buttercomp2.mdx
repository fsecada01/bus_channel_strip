...
| **Optical** | Vocals, acoustic instruments, bass | Soft knee, program-dependent release, warm and musical |
| **FET** | Snare, room mics, aggressive buss | Forward, punchy, saturated; behaves like a pushed 1176 |

**Classic** is the right starting point if you want compression you barely notice but always miss when it's bypassed. **VCA** excels when you need a quick, controllable transient response. **Optical** smooths out expressive performances without clamping down unnaturally. **FET** adds the kind of forward character that makes individual buses cut through.
...
<Tabs>
  ...
  <TabItem label="Bass DI Smoothing">
    | Control | Value |
    |---------|-------|
    | Threshold | `-14` dB |
    | Character % | `60` |
    ...
    Evens out note-to-note level variation common in fingerstyle and slap bass. Character adds warmth that complements direct-recorded bass tone.
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Snare Bus">
    ...
    Short attack grabs the snap; moderate release lets the snare ring naturally. Compensate output to match bypass level, then increase Mix until the snare sits forward in the mix.
  </TabItem>
